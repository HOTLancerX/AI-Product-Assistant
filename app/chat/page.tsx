...
export default function Chat() {
  ...
  const productsData: Product[] = [
    {
      ...
      shortDescription: "Premium 4K Smart TV with HDR support and voice control.",
      description: "Full description...",
      features: ["4K Resolution", "HDR10+", "Smart TV", "Voice Control"],
      specifications: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      analyzeAIResponse(aiResponse.content);
    } catch (err) {
  ...
  const generateAIResponse = (userInput: string): string => {
    // This would be generated by your AI in a real app
    return `
      <!-- PRIMARY_PRODUCT_ID:tv-samsung-55-4k -->
      <h2>Best Match For Your Needs</h2>
      <p>Based on your request for a high-quality TV, I recommend the <strong>Samsung 55" 4K Smart TV</strong>.</p>
      
      <h2>Alternative Options</h2>
      <ul>
        <li><strong>LG 43" Full HD Smart TV</strong> - More budget-friendly option</li>
        <li><strong>Sony 65" 4K OLED TV</strong> - Premium alternative with superior picture quality</li>
      </ul>
    `;
  };
  ...
  const analyzeAIResponse = (response: string) => {
    ...
    const primaryId = primaryMatch ? primaryMatch[1] : productsData[0].id;
    ...
    const primaryProduct = productsData.find(p => p.id === primaryId) || productsData[0];
    const alternatives = productsData
      .filter(p => p.id !== primaryProduct.id && mentionedIds.includes(p.id))
      .slice(0, 2);

    setRecommendation({
      primary: primaryProduct,
      alternatives: alternatives.length > 0 ? alternatives : productsData.slice(1, 3)
    });

    // Generate insights
    setAiInsights({
      confidence: 92,
      userIntent: "Product Recommendation",
      requirements: {
        budget: { max: 800 },
